...
public class AStarCar extends HeuristicSearchMethod {

	public AStarCar(EvaluationFunction function){
		super(function);
	}
	...
	public Node search(Problem problem, State initialState) {
		...
		while (!solutionFound && !frontier.isEmpty()) {			
			...
			//generatedStates.add(firstNode.getState());
			...
			//If the first node doesn't contain a problem's final state				
				...
				if (!generatedStates.contains(firstNode))
				{
				generatedStates.add(firstNode.getState());
